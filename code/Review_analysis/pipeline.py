...
def analyze_single_review(restaurant_id, restaurant_name, review_text):
    ...
    system_prompt = f"""너는 식당 리뷰 전문 분석 AI야. 주어진 하나의 리뷰 텍스트를 분석해서 반드시 아래 JSON 형식으로만 응답해줘. 다른 말은 덧붙이지 마.
리뷰에 직접적으로 언급되지 않은 항목(예: 청결도, 서비스 등)이 있다면, 리뷰의 전반적인 분위기를 바탕으로 유추하여 1~10 사이의 점수를 부여해줘.
...
{{
  ...
  "scores": {{
    "taste": 맛 점수 (1~10),
    "service": 서비스 점수 (1~10),
    "cleanliness": 청결도 점수 (1~10),
    "atmosphere": 분위기 점수 (1~10),
    "value": 가성비 점수 (1~10)
  }},
